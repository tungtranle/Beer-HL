...
Luôn tuân thủ AQF 4.1:
- Không tạo test gọi legacy destructive endpoints `reset-data`, `load-scenario`, `run-scenario`, `run-all-smoke`.
- Test cần data DB phải dùng scoped scenario API hoặc fixture read-only.
- Test scenario mutation phải assert `historical_rows_touched = 0` khi backend trả counter.
- Business-rule tests phải assert body/state/event, không chỉ status code.
...
- Pre-request: dùng fixture read-only hoặc scoped QA scenario ownership; KHÔNG gọi legacy reset/load scenario
...
- Mỗi test cần data phải dùng scoped scenario API hoặc fixture read-only; tuyệt đối không gọi test-portal reset legacy
- Nếu gọi scoped scenario API, assert data safety counter `historical_rows_touched = 0`
