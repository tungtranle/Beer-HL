...
HAS_CHIEF_ACCOUNTANT=$(docker exec "$CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" -tA -c \
    "SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='users' AND column_name='is_chief_accountant');" | tr -d '[:space:]')
...
if [ "$HAS_CHIEF_ACCOUNTANT" = "t" ]; then
cat >> "$TMP_FILE" <<'EOF'
,  is_chief_accountant
EOF
fi
