...
func NewZaloAdapter(accessToken, oaID string, mockMode bool) *ZaloAdapter {
	return &ZaloAdapter{
...
func (a *ZaloAdapter) SendZNS(ctx context.Context, msg ZNSMessage) (*ZNSResponse, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, "https://business.openapi.zalo.me/message/template", bytes.NewReader(body))
	if err != nil {
	...
	log.Printf("[Zalo] ZNS sent to %s: msg_id=%s", msg.Phone, result.Data.MsgID)
	return &result, nil
}
