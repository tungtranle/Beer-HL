...
export default function DriverDocumentsPage() {
  ...
  const load = () => {
    setLoading(true)
    apiFetch<any>(`/drivers/${driverId}/documents`)
      .then((r) => {
        const data = r.data || []
        setDocs(data)
    ...
    if (!driverName) {
      apiFetch<any>(`/drivers/${driverId}`)
        .then((r) => setDriverName(r.data?.full_name || r.data?.name || ''))
        .catch(() => {})
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'create') {
        await apiFetch(`/drivers/${driverId}/documents`, { method: 'POST', body: form })
      } else if (modal === 'edit' && editId) {
        await apiFetch(`/drivers/${driverId}/documents/${editId}`, { method: 'PUT', body: form })
      }
      setModal(null)
      load()
    } catch (e: any) {
  ...
  const handleDelete = async (id: string, docType: string) => {
    ...
    try {
      await apiFetch(`/drivers/${driverId}/documents/${id}`, { method: 'DELETE' })
      load()
    } catch (e: any) {
      toast.error(e.message)
    }
    ...
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800">📄 Giấy tờ tài xế {driverName}</h1>
        </div>
            ...
            <div key={d.id} className="bg-white rounded-xl shadow-sm border p-5">
              <div className="flex items-center justify-between mb-3">
                <span className="font-semibold text-gray-800">
                  ...
                  {d.license_class && <span className="ml-2 text-sm text-gray-500">({d.license_class})</span>}
                </span>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="grid gap-4">
              ...
              {form.doc_type === 'license' && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Hạng bằng lái</label>
                  <select value={form.license_class} onChange={e => setForm({ ...form, license_class: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm">
