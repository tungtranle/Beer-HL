...
# 14E. COMPONENT DESIGN SYSTEM

> **Sprint:** 30/04/2026 — Thiết lập hệ thống component chuẩn hóa cho toàn bộ frontend.  
> **Nguyên tắc:** Single source of truth cho UI primitives — mọi trang đều dùng từ `@/components/ui`, không tự viết ad-hoc.

## Mục tiêu

Thống nhất UI primitives, loại bỏ 200+ ad-hoc button/card/input/modal/spinner rải rác trong 49 trang dashboard, enforce qua ESLint (warn-only để migration dần).

## Thư viện primitives (`@/components/ui`)

| Component | Mô tả | Thay thế |
|-----------|-------|----------|
| `Button` | variant: primary/secondary/ghost/danger/success/subtle; size: sm/md/lg; loading state | 200+ `<button className=...>` ad-hoc |
| `Card` / `CardHeader` | variant: default/elevated/inset/interactive | 100+ `bg-white rounded-xl shadow-sm` |
| `PageHeader` | title+subtitle+icon+actions+leading+sticky | 50+ `<h1 className="text-2xl font-bold">` |
| `KpiCard` | Metric card với trend indicator | KPI metric blocks |
| `EmptyState` | Empty list placeholder với icon + CTA | Ad-hoc empty messages |
| `LoadingState` | Page/section/inline loading spinner | 90+ spinner divs |
| `StatusChip` | Status với dot màu + countdown | Inline status badges |
| `Skeleton` variants | SkeletonLine/Avatar/Card/TableRow/Grid/WithSkeleton | Ad-hoc skeleton blocks |
| `Pagination` | Phân trang chuẩn | Ad-hoc pagination buttons |
| `Modal` | ✅ MỚI — overlay portal, Esc close, backdrop click, scroll lock; size sm/md/lg/xl/full | Ad-hoc dialog divs |
| `ConfirmDialog` | ✅ MỚI — wraps Modal, danger variant, onConfirm/onCancel, confirmLabel/cancelLabel | `window.confirm()` calls |
| `Input` | ✅ MỚI — label/error/hint/prefixIcon/suffixIcon, passthrough HTML input props | 24+ raw `<input>` elements |
| `Textarea` | ✅ MỚI — label/error/hint, resize control | Raw `<textarea>` elements |
| `FormField` | ✅ MỚI — wrapper cho custom inputs với label + error/hint layout nhất quán | Inline label+input blocks |
| `Tabs` | ✅ MỚI — controlled tab bar + panels; tabs array `{key, label, icon?, badge?}`; variant: line/pill | 21+ ad-hoc `setTab` implementations |
| `Badge` | ✅ MỚI — count badge (khác StatusChip); tone: brand/info/success/warning/danger/neutral; size: sm/md | Hardcoded count badges |
| `Tooltip` | ✅ MỚI — hover tooltip pure CSS+state; position: top/bottom/left/right | `title` attribute / ad-hoc tooltip divs |
| `Alert` | ✅ MỚI — inline banner; tone: info/success/warning/danger; title, dismissible, actions slot | `bg-yellow-50` hardcoded banners |

## ESLint Enforcement (`.eslintrc.json`)

Rules `no-restricted-syntax` (warn-only — migration dần) banning:
- Raw `<input>` element → dùng `<Input>` từ `@/components/ui`
- Raw `<textarea>` element → dùng `<Textarea>`
- Raw `<select>` element → dùng `<FormField>` + custom select
- `window.confirm()` → dùng `<ConfirmDialog>`
- Ad-hoc spinner `animate-spin` trong `<div>` → dùng `<LoadingState>`

## Acceptance Criteria

- [x] 18 primitives đều export từ `@/components/ui/index.ts` — single import surface
- [x] TypeScript check `npx tsc --noEmit` passed — 0 errors
- [x] Modal dùng `createPortal` render tại `document.body` — tránh z-index conflicts
- [x] ConfirmDialog thay thế `window.confirm()` — không bị browser block
- [x] Tabs là controlled component — không có internal state
- [x] ESLint rules cài ở `warn` — code cũ không bị break, cảnh báo để migration dần
- [ ] COMP-010 — Migration: áp dụng cho 49 trang dashboard hiện tại (Sprint sau)

## Migration Backlog

Toàn bộ 49 trang dashboard hiện tại chưa sử dụng đầy đủ các primitives mới (COMP-010 — **TODO Sprint sau**). Ưu tiên migrate theo tần suất sử dụng cao nhất:
1. Các form nhập đơn hàng (OMS) — thay `<input>` → `<Input>`, modal → `<Modal>`
2. Các trang approval — thay `window.confirm()` → `<ConfirmDialog>`
3. Các trang có tabs ad-hoc — thay → `<Tabs>`
4. Các banner cảnh báo hard-code → `<Alert>`
